import { Context, createContext, Endian } from './common'
import {
    readBigInt64,
    readBigUint64,
    readBytes,
...
import { lensPath, set, lensProp, compose, over, apply, add } from 'ramda'
...
export const createParserContext = <T>(buffer: ArrayBuffer, offset = 0): ParserContext<T> => ({
    ...createContext(buffer),
    offset,
    values: {},
})

export const parse = <T>(fn: ParseFunction<T>) => (buffer: ArrayBuffer, offset = 0) =>
    fn(createParserContext(buffer, offset)).values

export const applyValue = <T, K extends keyof T = keyof T>([value, length]: ValueLengthTuple<T[K]>) => (prop: K) =>
    compose<ParserContext<T>, ParserContext<T>, ParserContext<T>>(
        over(lensProp('offset'), add(length)),
        set(lensPath(['values', prop as string]), value),
    )

export const derive = set(lensProp('values'), {})

export const applyArrayValue = <T, V = T>(prop: keyof T) => (parentContext: ParserContext<T>) => (
    ...
    values: {
        ...
        [prop]:
            typeof parentContext.values[prop] === 'undefined' || !Array.isArray(parentContext.values[prop])
                ? [context.values]
                : [...(parentContext.values[prop] as any), context.values],
    },
})

export const readValue = (fn: ReadFunction<any>) => <T>(prop: keyof T) => (
    context: ParserContext<T>,
): ParserContext<T> => applyValue<T>(fn({ buffer: context.buffer, offset: context.offset }))(prop)(context)

export const bytes = <T>(prop: keyof T) => (length: number) => readValue(readBytes(length))(prop)
...
export const fixedString = (encoding: string) => (length: number) => readValue(readFixedString(encoding)(length))

export const fixedAsciiString = fixedString('ascii')
export const fixedUtf8String = fixedString('utf8')

export const skip = <T>(length: number) => (context: ParserContext<T>): ParserContext<T> => ({
    ...context,
    offset: context.offset + length,
})

export const sequence = <T>(...fns: Array<ParseFunction<T>>) => (context: ParserContext<T>): ParserContext<T> =>
    fns.reduce((ctx, fn) => fn(ctx), context)

export const push = <V>(fn: ParseFunction<V>) => <T>(prop: keyof T) => (ctx: ParserContext<T>): ParserContext<T> =>
    apply(applyArrayValue<T, V>(prop)(ctx), fn({ ...((ctx as any) as ParserContext<V>), values: {} as Partial<V> }))

export const repeat = (length: number) => <V>(fn: ParseFunction<V>) => <T>(prop: keyof T) =>
    sequence(...Array.from({ length }, () => push(fn)(prop)))

export const repeatToEnd = <V>(fn: ParseFunction<V>) => <T>(prop: keyof T): ParseFunction<T> =>
    sequence(push(fn)(prop), ctx => (ctx.offset < ctx.buffer.byteLength ? repeatToEnd<V>(fn)(prop)(ctx) : ctx))

export const clear = <T>(context: ParserContext<T>): ParserContext<T> => ({ ...context, values: {} })

export const use = <T>(handler: (values: T) => ParseFunction<T>) => (context: ParserContext<T>): ParserContext<T> =>
    handler(context.values as T)(context)

export const when = <V extends string | number | symbol>(value: V) => <T>(
    cases: { [K in V]: ParseFunction<any> },
): ParseFunction<T> | undefined => cases?.[value]
...
export const logProp = <T>(prop: keyof T) => log<T>(ctx => `${prop}: ${ctx.values[prop]} [${ctx.offset}]`)
