...
export const pngRgbValue = sequence<PngRgbValue>(
    uint8('red'),
    uint8('green'),
    uint8('blue'),
);
...
export const pngPhysicalDimensions = sequence<PngPhysicalDimensions>(
    uint32Be('x'),
    uint32Be('y'),
    uint8('unit'),
);
...
export const pngChunk = sequence<PngChunk>(
    ...
    use(({ type, length }: PngChunk) => when(type.toLowerCase())({
        ihdr: pngHeader,
        plte: pngPalette,
